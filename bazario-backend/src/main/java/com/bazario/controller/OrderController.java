...
public class OrderController {
    ...
    /** Operateur & Admin — commandes already accepted, tracked through the delivery lifecycle */
    @GetMapping("/suivi")
    @PreAuthorize("hasRole('OPERATEUR') or hasRole('ADMIN')")
    public ResponseEntity<Page<OrderDto.Response>> getSuivi(
            @RequestParam(required = false) Order.OrderStatus status,
            @RequestParam(required = false) String q,
            @RequestParam(defaultValue = "0") @Min(0) int page,
            @RequestParam(defaultValue = "20") @Min(1) @Max(200) int size,
            @RequestParam(defaultValue = "updatedAt") String sort,
            @RequestParam(defaultValue = "desc") String sortDir) {
        return ResponseEntity.ok(orderService.getSuiviPaged(status, q, page, size, sort, sortDir));
    }
